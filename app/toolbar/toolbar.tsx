...
export const MainToolbarNoOutlet: React.FC<{
  ...
  var list = [
    ...
    [{ pathname: "/props", name: "Props" }],
    [{ pathname: "/todo_chars", name: "Characters" }],
  ...
  if (getConflictingChars(knownProps, characters).length > 0) {
    list.push([{ pathname: "/conflicts", name: "Conflicts" }]);
  }
            ...
            {list.map((items, i) => {
              return (
                <li key={i} className="flex ">
                  {items.map((item, j) => {
                    let isCurrent = item.pathname === location.pathname;
                    return (
                      <NavLink
                        key={j}
                        to={item.pathname}
                        className={isCurrent ? styleSelected : styleInactive}
                      >
                        {item.name}
                      </NavLink>
                    );
