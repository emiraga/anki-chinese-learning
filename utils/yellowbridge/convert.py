...
def extract_functional_components(decomp_html: str) -> dict[str, list[dict[str, Any]]]:
    ...
    """
    components: dict[str, list[dict[str, Any]]] = {"phonetic": [], "semantic": [], "primitive": []}
    ...
    for pattern, comp_type in component_patterns:
        ...
        for char, pinyin, description in matches:
            ...
            component_data = {"character": char, "pinyin": pinyin_list, "description": desc_clean}
...
def extract_character_info(decomp_html: str) -> dict[str, Any] | None:
    ...
    if match:
        ...
        return {"character": match.group(1).strip(), "pinyin": pinyin_list, "definition": match.group(3).strip()}
...
def extract_radical(decomp_html: str) -> dict[str, Any] | None:
    ...
    if match:
        ...
        result = {"character": char, "pinyin": pinyin_list, "description": description}
...
def extract_simplification(formation_html: str) -> dict[str, str] | None:
    ...
    if match:
        ...
        if simplified_match:
            result = {"simplifiedForm": simplified_match.group(1).strip(), "method": re.sub(r"<[^>]+>", "", content).strip()}
            if method_type:
...
def process_file(file_path: Path) -> dict[str, Any]:
    ...
    if char_info and char_info["character"] != filename_char:
        raise ValueError(f"Character mismatch: filename '{filename_char}' != content '{char_info['character']}'")
...
def build_sounds_component_index(results: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for char, data in results.items():
        ...
        for phonetic in phonetic_components:
            ...
            usage_info = {"character": char, "pinyin": data.get("pinyin", [])}
...
def process_directory(
    input_dir: Path, output_file: Path | None = None, individual_files_dir: Path | None = None
...
def main():
    ...
    parser.add_argument("--no-individual", action="store_true", help="Skip creating individual character files")
